...
local function note(t, m, d)
  _G.__WOODZ_UTILS.notify(t, m, d or 3)
end
...
local serverHopper = r("server_hopper")
local solo         = r("solo")
...
function App.start()
  if _G.__WOODZ_APP_STARTED then
    return
  end
  ...
  local h = {
    ...
    picker_clear       = (farm and function()
      farm.setSelected({})
    end) or (smart and smart.clear),

    -- Farm toggles
    onAutoFarmToggle = (profile.ui.autoFarm and function(v)
      local on = (v ~= nil) and v or false
      if on and farm and farm.setupAutoAttackRemote then
        farm.setupAutoAttackRemote()
      end
      if farm and farm.runAutoFarm then
        stopAutoFarm()
        autoFarmThread = task.spawn(function()
          farm.runAutoFarm(function()
            return on
          end, App.UI and App.UI.setCurrentTarget)
        end)
      end
    end) or nil,
    ...
    onSmartFarmToggle = (profile.ui.smartFarm and function(v)
      local on = (v ~= nil) and v or false
      if smart and smart.toggle then
        smart.toggle(on)
      end
    end) or nil,
    ...
    onToggleMerchant2 = (profile.ui.merchants and function(v)
      ...
      if merchants and merchants.autoBuyLoop and on then
        task.spawn(function()
          merchants.autoBuyLoop("SmelterMerchantService2", function()
            return on
          end, function() end)
        end)
      end
    ...
    onToggleCrates = (profile.ui.crates and function(v)
      ...
      if crates and crates.autoOpenCratesEnabledLoop and on then
        task.spawn(function()
          crates.autoOpenCratesEnabledLoop(function()
            return on
          end)
        end)
      end
    ...
    onRedeemCodes = (profile.ui.redeemCodes and function()
      if redeem and redeem.run then
        ...
        note("Codes", "redeem_unredeemed_codes.lua missing", 4)
      end
    end) or nil,
    ...
    onFastLevelToggle = (profile.ui.fastlevel and function(v)
      ...
      if fastOn then
        ...
        pcall(function()
          if farm and farm.setFastLevelEnabled then
            farm.setFastLevelEnabled(true)
          end
        end)
        autoFarmOn = true
        if App.UI and App.UI.setAutoFarm then
          pcall(App.UI.setAutoFarm, true)
        end
        if farm and farm.runAutoFarm then
          stopAutoFarm()
          autoFarmThread = task.spawn(function()
